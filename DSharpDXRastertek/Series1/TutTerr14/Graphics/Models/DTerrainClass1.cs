...
namespace DSharpDXRastertek.TutTerr14.Graphics.Models
{
    public class DTerrain                   // 509 lines
    {
        // Structs
        ...
        public DTexture GrassTexture { get; set; }
        public DTexture SlopeTexture { get; set; }
        public DTexture RockTexture { get; set; }

        // Constructor
        public DTerrain() { }

        // Methods.
        public bool Initialize(SharpDX.Direct3D11.Device device, string heightMapFileName, string grassTextureFilename, string slopeTextureFilename, string rockTextureFilename)
        {
        ...
        {
            grassTextureFilename = DSystemConfiguration.DataFilePath + grassTextureFilename;
            slopeTextureFilename = DSystemConfiguration.DataFilePath + slopeTextureFilename;
            rockTextureFilename = DSystemConfiguration.DataFilePath + rockTextureFilename;

            // Create the texture object.
            GrassTexture = new DTexture();

            // Initialize the texture object.
            if (!GrassTexture.Initialize(device, grassTextureFilename))
                return false;

            // Create the slope texture object.
            SlopeTexture = new DTexture();

            // Initialize the slope texture object.
            if (!SlopeTexture.Initialize(device, slopeTextureFilename))
                return false;

            // Create the rock texture object.
            RockTexture = new DTexture();

            // Initialize the slope texture object.
            if (!RockTexture.Initialize(device, rockTextureFilename))
                return false;
