...
namespace DSharpDXRastertek.Tut32.Graphics.Shaders
{
    public class DGlassShader                   // 266 lines
    {
        // Structs
        ...
        public SharpDX.Direct3D11.Buffer ConstantGlassBuffer { get; set; }
        ...
        public bool Initialize(Device device, IntPtr windowsHandler)
        {
            ...
            return InitializeShader(device, windowsHandler, "glass.vs", "glass.ps");
        }
        private bool InitializeShader(Device device, IntPtr windowsHandler, string vsFileName, string psFileName)
        {
            try
            {
                ...
                ShaderBytecode vertexShaderByteCode = ShaderBytecode.CompileFromFile(vsFileName, "GlassVertexShader", DSystemConfiguration.VertexShaderProfile, ShaderFlags.None, EffectFlags.None);
                ShaderBytecode pixelShaderByteCode = ShaderBytecode.CompileFromFile(psFileName, "GlassPixelShader", DSystemConfiguration.PixelShaderProfile, ShaderFlags.None, EffectFlags.None);
                ...
                // Setup the description of the dynamic matrix constant Matrix buffer that is in the vertex shader.
                BufferDescription glassBufferDescription = new BufferDescription()
                {
                    Usage = ResourceUsage.Dynamic,
                    SizeInBytes = Utilities.SizeOf<DGlassBuffer>(),
                    BindFlags = BindFlags.ConstantBuffer,
                    CpuAccessFlags = CpuAccessFlags.Write,
                    OptionFlags = ResourceOptionFlags.None,
                    StructureByteStride = 0
                ...
                // Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
                ConstantGlassBuffer = new SharpDX.Direct3D11.Buffer(device, glassBufferDescription);
        ...
        private void ShuddownShader()
        {
            ...
            ConstantGlassBuffer?.Dispose();
            ConstantGlassBuffer = null;
            // Release the matrix constant buffer.
        ...
        public bool Render(DeviceContext deviceContext, int indexCount, Matrix worldMatrix, Matrix viewMatrix, Matrix projectionMatrix, ShaderResourceView colorTexture, ShaderResourceView normalTexture, ShaderResourceView refractionTexture, float refractionScale)
        {
