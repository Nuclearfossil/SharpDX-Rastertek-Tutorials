...
namespace DSharpDXRastertek.TutTerr09.Graphics.Models
{
    public class DText                  // 459 lines
    {
        ...
        public bool Initialize(SharpDX.Direct3D11.Device device, DeviceContext deviceContext, IntPtr windowHandle, int screanWidth, int screenHeight, Matrix baseViewMatrix)
        {
            ...
            if (!InitializeSentence(out sentences[3], 24, 90, device))
                return false;
        ...
        public bool SetCpu(int cpu, int cpu0,  DeviceContext deviceContext)
        {
            // Format string for this sentance to report CPU Usage percetange
            string formattedCpuUsage = string.Format("CPU: ALL- {0}%  1- {1}%", cpu.ToString().PadLeft(3), cpu0.ToString().PadLeft(3));
