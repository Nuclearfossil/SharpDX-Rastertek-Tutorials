...
namespace DSharpDXRastertek.TutTerr09.Graphics.Shaders
{
    public class DTerrainShader                 // 329 lines
    {
        // Structures
        ...
        [StructLayout(LayoutKind.Sequential)]
        internal struct DTextureInfoBufferType
        {
            internal bool useAlplha;
            internal Vector3 padding2;
        }
        ...
        public SharpDX.Direct3D11.Buffer ConstantTextureInfoBuffer { get; set; }
        ...
        private bool InitializeShader(Device device, IntPtr windowHandler, string vsFileName, string psFileName)
        {
            try
            {
                ...
                BufferDescription textureInfoBufferDesc = new BufferDescription()
                {
                    Usage = ResourceUsage.Dynamic,
                    SizeInBytes = Utilities.SizeOf<DTextureInfoBufferType>(), // Must be divisable by 16 bytes, so this is equated to 32.
                    BindFlags = BindFlags.ConstantBuffer,
                    CpuAccessFlags = CpuAccessFlags.Write,
                    OptionFlags = ResourceOptionFlags.None,
                    StructureByteStride = 0
                ...
                ConstantTextureInfoBuffer = new SharpDX.Direct3D11.Buffer(device, textureInfoBufferDesc);
        ...
        private void ShuddownShader()
        {
            ...
            ConstantTextureInfoBuffer?.Dispose();
            ConstantTextureInfoBuffer = null;
        ...
        public bool SetShaderTextures(DeviceContext deviceContext, ShaderResourceView texture1, ShaderResourceView texture2, ShaderResourceView alphaMap, bool useAlpha)
        {
            // Set shader texture resources in the pixel shader.
            deviceContext.PixelShader.SetShaderResources(0, 1, texture1);

            // If this is a blended polygon then also set the second texture and the alpha map for blending.
            if (useAlpha)
            {
                deviceContext.PixelShader.SetShaderResources(1, 1, texture2);
                deviceContext.PixelShader.SetShaderResources(2, 1, alphaMap);
            }

            // Lock the texture info constant buffer so it can be written to.
            DataStream mappedResource;
            deviceContext.MapSubresource(ConstantTextureInfoBuffer, MapMode.WriteDiscard, MapFlags.None, out mappedResource);

            // Copy the texture info variables into the constant buffer.
            DTextureInfoBufferType textureInfouffer = new DTextureInfoBufferType()
            {
                useAlplha = useAlpha,
                padding2 = Vector3.Zero
            };
            mappedResource.Write(textureInfouffer);

            // Unlock the constant buffer.
            deviceContext.UnmapSubresource(ConstantTextureInfoBuffer, 0);

            // Set the position of the texture info constant buffer in the pixel shader.
            int bufferPositionNumber = 1;

            ;// Finally set the texture info constant buffer in the pixel shader with the updated values.
            deviceContext.PixelShader.SetConstantBuffer(bufferPositionNumber, ConstantTextureInfoBuffer);
