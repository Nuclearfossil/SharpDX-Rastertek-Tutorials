...
namespace DSharpDXRastertek.TutTerr05.Graphics.Models
{
    public class DText                  // 402 lines
    {
        ...
        {
            // Format string for this sentance to report CPU Usage percetange
            string formattedRenderCount = string.Format("Triangle Count: {0}", renderCount.ToString().PadLeft(7));

            // Update the sentence vertex buffer with the new string information.
            if (!UpdateSentece(ref sentences[10], formattedRenderCount, 20, 260, 0, 1, 0, deviceContext))
                return false;
