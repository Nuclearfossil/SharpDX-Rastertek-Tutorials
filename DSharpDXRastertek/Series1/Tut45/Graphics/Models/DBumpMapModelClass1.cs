...
namespace DSharpDXRastertek.Tut45.Graphics.Models
{
    public class DBumpMapModel                  // 353 lines
    {
        ...
        public DTexture ColorTexture { get; set; }
        public DTexture NormalMapTexture { get; set; }

        // Constructor
        public DBumpMapModel() { }

        // Methods
        public bool Initialize(SharpDX.Direct3D11.Device device, string modelFormatFilename, string colorTextureFileName, string normalMapTextureFileName)
        {
            ...
            if (!LoadTextures(device, colorTextureFileName, normalMapTextureFileName))
                return false;
        ...
        private bool InitializeBuffers(SharpDX.Direct3D11.Device device)
        {
            try
            {
                ...
                DBumpMapShader.DVertex[] vertices = new DBumpMapShader.DVertex[VertexCount];          
                // Create the index array.
                var indices = new int[IndexCount];

                for (var i = 0; i < VertexCount; i++)
                {
                    vertices[i] = new DBumpMapShader.DVertex()
                    {
        ...
        private void RenderBuffers(SharpDX.Direct3D11.DeviceContext deviceContext)
        {
            ...
            deviceContext.InputAssembler.SetVertexBuffers(0, new VertexBufferBinding(VertexBuffer, Utilities.SizeOf<DBumpMapShader.DVertex>(), 0));
