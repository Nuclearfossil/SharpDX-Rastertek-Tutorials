...
namespace DSharpDXRastertek.Series2.TutTerr14.Graphics.Shaders
{
    public class DShaderManager
    {
        ...
        public bool RenderTerrainShader(DeviceContext deviceContext, int indexCount, Matrix worldMatrix, Matrix viewMatrix, Matrix projectionMatrix, ShaderResourceView texture, ShaderResourceView normal, ShaderResourceView normal1, ShaderResourceView normal2, Vector3 lightDirection, Vector4 diffuse)
        {
            if (!TerrainShader.Render(deviceContext, indexCount, worldMatrix, viewMatrix, projectionMatrix, texture, normal, normal1, normal2,  lightDirection, diffuse))
                return false;
